...
async function initializeDatabase() {
  try {
    ...
    if (existingTables.rows.length > 0) {
      console.log("⚠️  Some tables already exist. Use 'npm run db:push' to sync schema changes.");
      return;
    }
    
    console.log("📦 Creating database schema...");
    console.log("ℹ️  Use 'npm run db:push' to create tables based on your schema");
    console.log("ℹ️  Or use 'npm run db:generate' and 'npm run db:migrate' for production");
    
    console.log("✨ Database initialization complete!");
    console.log("\n🎯 Next steps:");
    console.log("1. Update your DATABASE_URL with your real Neon connection string");
    ...
    if (error.message.includes("authentication failed")) {
      console.error("🔐 Authentication issue - check your DATABASE_URL credentials");
